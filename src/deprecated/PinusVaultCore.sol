...
contract PinusVaultCore is SwapAbstract,ERC20,Ownable {
    string _name;
    string _symbol;
    ...
    constructor(address owner, string memory newName, string memory newSymbol) Ownable(owner) {
        _name = newName;
        _symbol = newSymbol;
    }
    ...
    function getERC20ToNativePrice(uint128 erc20Sold) public virtual view returns (uint128 nativeBought) {

    }

    function erc20ToNativeSwap(uint128 erc20Sold, address recipient) public virtual returns (uint128 nativeBought) {
        
    }

    function availableERC20() returns (uint128) {
        return (block.timestamp - erc20StartTime) * erc20Rate;
    }

    function getNativeToERC20Price(uint128 nativeSold) public override view returns (uint128 erc20Bought) {
        return availableERC20() * nativeSold / (nativeInhibitor + nativeSold);
    }

    function nativeToERC20Swap(address recipient) public override payable returns (uint128 erc20Bought) {
